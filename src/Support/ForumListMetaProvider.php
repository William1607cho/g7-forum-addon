 ...
 *  - `last_activity_at`(ISO) / `last_activity_at_formatted`(표시용) : 게시글 자신의
 *    작성 시각과 그 게시글에 달린(소프트 삭제 제외) 모든 댓글의 작성 시각을 통틀어
 *    가장 최근 값. 표시 포맷은 sirsoft-board 의 `FormatsBoardDate` 트레이트를 그대로
...
class ForumListMetaProvider
{
    ...
    public function batch(int $boardId, array $postIds, string $dateFormat = 'standard'): array
    {
        ...
        // 1) 게시글별 최종 활동 시각 — 게시글 자신 + 댓글 전체(소프트 삭제 제외) 통틀어 최댓값.
        //    user_id 유무와 무관하게 항상 계산되도록(익명/탈퇴 작성자도 시각은 유효) 별도 쿼리로 분리.
        $lastActivityRows = DB::select(
            "SELECT post_id, MAX(created_at) AS last_activity_at FROM (
                SELECT post_id, created_at FROM {$commentsTable}
                WHERE board_id = ? AND post_id IN ({$placeholders}) AND deleted_at IS NULL
                UNION ALL
                SELECT id AS post_id, created_at FROM {$postsTable}
        ...
        $lastActivityByPost = [];
        foreach ($lastActivityRows as $row) {
            $lastActivityByPost[(int) $row->post_id] = $row->last_activity_at;
        }
