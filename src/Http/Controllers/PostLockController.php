 ...
 *
 * ── 권한 (1차 범위) ─────────────────────────────────────────────
 * `AdminBaseController` 가 `auth:sanctum` + `admin` 미들웨어를 건다 → **사이트 관리자
 * (`User::isAdmin()`)만** 잠글 수 있다. 가장 보수적인 기본값이다. 게시글 작성자 본인의
 * 셀프 잠금, per-board 매니저 위임은 향후 확장 여지로 남긴다(보고서 참조).
 *
 * ── 가시성 재검증 ──────────────────────────────────────────────
 ...
 */
class PostLockController extends AdminBaseController
{
    ...
    /**
     * 공통 처리 — 가시성/포럼유형 재검증 후 메타 행 upsert.
     */
